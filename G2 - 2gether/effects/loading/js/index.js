...
//setTimeout("location.href = 'http://127.0.0.1:5500/index.html';",2500;
...
// Build the output array.
  ...
  flyway = function(){
    ...
    jqElement
      .off()
      .addClass('body-fly-away')
      jqElement.on('animationend webkitAnimationEnd oAnimationEnd MSAnimationEnd', function(event) {
        jqElement.remove();
        setTimeout("location.href = 'https://playtwogether.herokuapp.com/index.html';",1000);
      });
  }

  initialise = function() {
    var index, j, len, text;
    $animate.click(flyway);
    var index, j, len, text;
    for (index = j = 0, len = MESSAGES.length; j < len; index = ++j) {
      text = MESSAGES[index];
      $message.append("<p>");
  ...
  setTimeout(function(){
    var x = document.getElementById("animate-message")
